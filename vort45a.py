...
DefineScalarExpression("pi", "3.14159265")
...
def func (theta,state, unused2):
	...
	pi=math.pi
	thetastr=str(theta)
	thetaanglestr=str(theta*180/pi)
	...
	amparr[12]=0.0004782
	print "state", state
	ampdef=math.exp(.75*state)*1.122e-6/math.exp(0.75*2)
	lx=2.0
	eps=1e-3
	sbomega=1e-3
	scrh=lx*eps*sbomega/8.0
	ampdef=math.exp(.75*state)*scrh
	...
	DefineScalarExpression("mytime", "exp( %f *0.75) " % state)
	#DefineScalarExpression("amp","scrh*exp(0.75*<time_derivative/mesh_time>)" )
	DefineScalarExpression("vmri", "amp*sin(2*pi*z)")
